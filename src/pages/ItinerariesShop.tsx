...
const ItinerariesShop = () => {
  ...
  const inputCls = "w-full px-3.5 py-3 text-[0.9rem] bg-voyage-white border border-ink/15 rounded-xs focus:outline-hidden focus:border-gold text-ink placeholder:text-voyage-muted/70 transition-colors";
  const filterSelectCls = "w-full px-3 py-2.5 text-[0.85rem] bg-voyage-white border border-ink/10 rounded-xs focus:outline-hidden focus:border-gold text-ink";
  ...
  return (
    <div className="min-h-screen bg-parchment">
      ...
      {/* FILTER BAR */}
      <section className="border-b border-ink/10 bg-voyage-white sticky top-0 z-30 shadow-xs">
      ...
      <main className="max-w-[1200px] mx-auto px-4 md:px-16 py-10 md:py-24">
        ...
        {!isLoading && (data?.length ?? 0) > 0 && filtered.length === 0 && (
          <div className="border border-ink/10 bg-voyage-white rounded-lg p-12 text-center">
            ...
            <button onClick={resetFilters} className="inline-block px-6 py-3 rounded-xs bg-ink text-voyage-white text-[0.72rem] font-medium tracking-[0.12em] uppercase hover:bg-gold hover:text-ink transition-colors">
              {t("catalogue.filters.reset")}
            </button>
          </div>
        )}
            ...
            return (
              <ScrollReveal key={trip.id} className="h-full">
                <Link
                  ...
                  className="group flex flex-col h-full w-full rounded-lg overflow-hidden border border-ink/[0.08] bg-voyage-white shadow-xs hover:shadow-xl transition-all duration-300 text-left"
                >
                      ...
                      return (
                        <div className="flex items-center flex-wrap gap-x-3 gap-y-1 mb-4 text-[0.62rem] tracking-[0.12em] uppercase text-voyage-muted">
                          <span className="inline-flex items-center gap-1.5">
                            ...
                            <span className="px-1.5 py-0.5 rounded-xs border border-ink/15 text-ink/80 font-semibold">
            ...
            {submitted ? (
              ...
              <form onSubmit={submitSuggestion} className="bg-voyage-white border border-parchment-3 rounded-lg p-7 md:p-10 flex flex-col gap-5">
                        ...
                        <span className="flex flex-wrap gap-1.5">
                          {sg.experience.length === 0 ? (
                            ...
                            sg.experience.map((x) => (
                              <span
                                key={x}
                                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-xs bg-gold/15 text-[0.75rem] text-ink"
                              >
                      ...
                      <div className="max-h-[260px] overflow-y-auto p-2">
                        {EXPERIENCE_OPTIONS.map((x) => {
                          ...
                          return (
                            <label
                              key={x}
                              className={`flex items-center gap-3 px-3 py-2 rounded-xs cursor-pointer transition-colors ${
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="self-start px-7 py-3.5 rounded-xs bg-gold text-ink text-[0.72rem] font-semibold tracking-[0.14em] uppercase hover:bg-ink hover:text-voyage-white transition-colors disabled:opacity-60"
                >
